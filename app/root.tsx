...
export async function loader({ request, params, context }: DataFunctionArgs) {
  ...
  const detectedLocale = await getI18NextServer().then((i18next) =>
    i18next.getLocale(request),
  );
  // Default to Arabic if no explicit language preference found
  const locale = detectedLocale === 'en' ? 'ar' : (detectedLocale || 'ar');
